...
# Объект бота
bot = Bot(token="6204814050:AAH4624kqEK11H03CJRXmHb-YXAF45TTA1w")
# Диспетчер
dp = Dispatcher(bot)
...
async def cmd_start(message: types.Message):
    ...
    async def button_booking(message: types.Message):
        ...
        # Calculate price based on number of hours and parking space selected
        price = time_hour * element[3]
        
        await bot.send_invoice(
            message.chat.id,
            title="Бронь места",
            description="Сейчас вы можете забронировать место на парковке, чтобы это сделать, вам надо заплатить {} рублей в качестве залога :)"
                        .format(price/10),
            provider_token='381764678:TEST:54163',
            currency='rub',
            ...
            photo_size=256,
            is_flexible=False,
            prices=[types.LabeledPrice(label="Бронирование парковочного места",
                                    amount=price*10)],
